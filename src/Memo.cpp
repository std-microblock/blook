...
namespace blook {
    ...
    std::optional<std::vector<uint8_t>> Pointer::try_read(void *ptr,
        ...
        const auto x = proc->read((void *) ((size_t) ptr + offset), size);
        return x;
    }
    ...
    std::expected<void, std::string> Pointer::write(void *addr, std::span<uint8_t> data) const {
        return proc->write((void *) ((size_t) addr + offset), data);
    }

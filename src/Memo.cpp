...
namespace blook {
    ...
    std::vector<uint8_t> Pointer::read(void *ptr, size_t size) {
        ...
        const auto x = proc->read((void *) ((size_t) ptr + offset), size).value();
        return x;
    }
    ...
    disasm::DisassembleIterator<std::span<uint8_t>> MemoryRange::disassembly() const {
        return disasm::DisassembleIterator<std::span<uint8_t>>{*this};
    }
    ...
    int32_t MemoryRange::crc32() const {
        ...
        for (size_t i = 0; i < _size; i++) {
            crc = crc32_table[(crc ^ *((uint8_t *) offset)) & 0xFF] ^ (crc >> 8);
        }
